...
namespace Bina.Areas.Manager.Controllers
{
    ...
    public class UsersController : Controller
    {
        ...
        public async Task<IActionResult> Index(string searchTerm, string sortBy, string sortDirection, int? pageSize = 5, int? pageNumber = 1)
        {
            ...
            // Lấy FacultyId của Manager hiện tại từ session
            string ManagerFacultyId = HttpContext.Session.GetString("FacultyId");

            // Lọc danh sách user theo FacultyId của Manager
            IQueryable<User> Ft1Context = _context.Users
                .Include(u => u.Faculty)
                .Include(u => u.Role)
                .Include(u => u.Terms)
                .Where(u => u.FacultyId == ManagerFacultyId && u.RoleId != 1);
        ...
        public IActionResult Create()
        {
            ...
            ViewData["FacultyName"] = new SelectList(_context.Faculties.Where(f => f.FacultyId == ManagerFacultyId), "FacultyId", "FacultyName");
            var allowedRoles = new List<string> { "Marketing Coordinator", "Student", "Guest" };
        ...
        public async Task<IActionResult> Create([Bind("UserName,FirstName,LastName,PhoneNumber,DoB,DateCreated,Gender,Email,Password,AvatarPath,RoleId,FacultyId,Terms.TermsText")] User user)
        {
            if (ModelState.IsValid)
            {
                ...
                // Kiểm tra xem FacultyId của user mới có trùng với FacultyId của Manager hay không
                if (user.FacultyId != ManagerFacultyId)
                {
                    ModelState.AddModelError("FacultyId", "User can only be created with the same FacultyId as the current Manager.");
